...
if	( !isset( $_REQUEST['lp_classe'] ) )
	...
	if	( !$class_name )
		...
		$paycnt = 1;		// le nombre max de versements, pour ajuster les colonnes
		...
		foreach	( $my_students as $k => $v ) {
			...
			// echo '<pre>'; var_dump( $paid[$v] ); echo '</pre>';
		...
		$html_css = '<style type="text/css">'
			...
			. 'table.lp td { border:1px solid black; padding: 2px 10px 2px 10px; text-align: right }'
		...
		echo $html_css, '<div id=pdfpage><h3>', $class_name, '</h3>';
		echo '<table class="lp">';
